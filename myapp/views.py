...
def base(request):
    context = {
        ...
        'total_sold_products': Product.objects.filter(is_sold=True).count(),
        'total_unsold_products': Product.objects.filter(is_sold=False).count(),
        'resolved_feedbacks': Feedback.objects.filter(resolved=True).count(),
        'unresolved_feedbacks': Feedback.objects.filter(resolved=False).count(),
    }
